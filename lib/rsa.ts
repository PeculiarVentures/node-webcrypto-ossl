...
export class RsaCrypto extends BaseCrypto {
    static generateKey(algorithm: RsaHashedKeyGenParams, extractable: boolean, keyUsages: string[]): PromiseLike<CryptoKeyPair> {
...
export class RsaPKCS1 extends RsaCrypto {
    ...
    static sign(algorithm: Algorithm, key: CryptoKey, data: Buffer): PromiseLike<ArrayBuffer> {
        return new Promise((resolve, reject) => {
    ...
    static verify(algorithm: Algorithm, key: CryptoKey, signature: Buffer, data: Buffer): PromiseLike<boolean> {
        return new Promise((resolve, reject) => {
            let _alg = this.wc2ssl(key.algorithm);
            let nkey = key.native as native.Key;
